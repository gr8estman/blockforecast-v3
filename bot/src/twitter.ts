...
export interface MentionTweet {
  id:          string;
  authorId:    string;
  mentionText: string; // only the mention tweet itself (for keyword matching)
  text:        string; // mention + parent tweet combined (for CA scanning)
}

export class XClient {
  private rw: TwitterApiReadWrite;
  private botUserId = "";
  ...
  async getMentions(sinceId?: string): Promise<MentionTweet[]> {
    ...
    const refMap   = new Map<string, string>(
      (includes.tweets ?? []).map((t: { id: string; text: string }) => [t.id, t.text])
    );
    ...
    return tweets
      ...
      .map(t => {
        const refTexts = (t.referenced_tweets ?? [])
          .map((r: { id: string }) => refMap.get(r.id) ?? "")
          .filter(Boolean)
          .join(" ");
        return {
          id:          t.id,
   ...
   */
  async replyWithThread(replyToTweetId: string, tweets: string[], imageUrl?: string): Promise<void> {
    ...
    for (let i = 0; i < tweets.length; i++) {
      ...
      try {
        const payload = i === 0 && mediaId
          ? { reply: { in_reply_to_tweet_id: parentId }, media: { media_ids: [mediaId] as [string] } }
  ...
  private async _uploadImageFromUrl(url: string): Promise<string> {
    const res = await fetch(url, { signal: AbortSignal.timeout(8_000) });
    if (!res.ok) throw new Error(`Image fetch failed: ${res.status}`);
    const contentType = res.headers.get("content-type") ?? "image/png";
    const mimeType = contentType.split(";")[0].trim() as "image/png" | "image/jpeg" | "image/gif" | "image/webp";
