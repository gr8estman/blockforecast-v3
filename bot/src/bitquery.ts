...
export async function fetchBasicStats(token: string): Promise<{
  price: number; volume24h: number; trades24h: number; marketCap: number;
} | null> {
  ...
  const data = await gql<{
    Solana: { DEXTradeByTokens: Array<{ Trade: { PriceInUSD: string | number; AmountInUSD: string | number } }> }
  }>(q);
...
export async function fetchSmartMoney(token: string): Promise<Array<{ address: string; label?: string; volumeUsd: number }>> {
  const since24h  = new Date(Date.now() - 24 * 3600_000).toISOString();
  const sideMints = SIDE_MINTS.map(m => `"${m}"`).join(", ");
  ...
  const wallets = new Map<string, { buyUsd: number; sellUsd: number }>();
  for (const t of data.Solana.DEXTradeByTokens) {
    const owner = t.Trade.Account.Owner;
    const usd   = Number(t.Trade.AmountInUSD) || 0;
    if (!wallets.has(owner)) wallets.set(owner, { buyUsd: 0, sellUsd: 0 });
    const w = wallets.get(owner)!;
    if (t.Trade.Side.Type === "buy") w.buyUsd += usd; else w.sellUsd += usd;
  }
