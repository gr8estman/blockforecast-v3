...
import { XClient } from "./twitter.js";
...
function loadState() {
    try {
        ...
        return { repliedTo: [], postsToday: 0, postDayReset: today() };
    }
}
...
async function processMention(xClient, tweet, state) {
    if (state.repliedTo.includes(tweet.id))
        return;
...
async function pollMentions(xClient, state) {
    try {
        ...
        for (const tweet of sorted) {
            await processMention(xClient, tweet, state);
            await sleep(8000);
        }
    }
    catch (err) {
